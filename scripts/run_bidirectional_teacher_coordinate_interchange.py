"""Execute one sealed C18 condition under an explicitly released manifest.
...
def batch_inputs(tokenizer, rows: list[dict], device: torch.device) -> dict[str, torch.Tensor]:
    width = max(len(row["input_ids"]) for row in rows)
    ids, masks = [], []
    for row in rows:
        padding = width - len(row["input_ids"])
        ids.append([int(tokenizer.pad_token_id)] * padding + list(row["input_ids"]))
        masks.append([0] * padding + [1] * len(row["input_ids"]))
    return {
...
def main() -> None:
    ...
    parser.add_argument("--authorization", required=True, help="Ignored runtime JSON created only after explicit release")
    ...
    seal_key = os.environ.get("SLGEO_C18_SEAL_KEY", "").encode("ascii")
    if not seal_key:
        raise RuntimeError("STOP: runtime-only C18 sealing key is absent")
    checked_inputs = validate_public_inputs(manifest, Path.cwd(), require_runtime_inputs=True)
    ...
    for start in range(0, len(inventory), 6):
        prompt_rows = inventory[start : start + 6]
        inputs = batch_inputs(tokenizer, prompt_rows, device)
