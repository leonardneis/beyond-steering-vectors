"""Render the versioned C18 technical DAG using only the Python standard library."""
...
def render(source: str, *, commit: str, start_epoch: int, ntfy_topic: str = "") -> str:
    ...
    if source.count('BsvExecutionGitCommit="UNFROZEN"') != 4:
        raise ValueError("versioned C18 DAG has an unexpected execution-commit inventory")
    if source.count('BsvStartEpoch="0"') != 1 or source.count('BsvNtfyTopic=""') != 1:
        raise ValueError("versioned C18 DAG has unexpected notification placeholders")
    return (
