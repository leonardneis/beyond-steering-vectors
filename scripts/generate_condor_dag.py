...
def render_dag(tasks: list[Task], manifest_path: str, manifest: dict, repo_root: str, shared_root: str, image: str) -> str:
    ...
    gpu_resource_attempts = int(manifest["condor"].get("gpu_resource_attempts", 4))
    if not 2 <= gpu_resource_attempts <= 4:
        raise ValueError("condor.gpu_resource_attempts must be between 2 and 4")
    ...
    for task in tasks:
        ...
        if task.gpus:
            variables["BsvGpuResourceAttempts"] = str(gpu_resource_attempts)
        lines.append("VARS " + task.task_id + " " + " ".join(f'{key}=\"{quote(value)}\"' for key, value in variables.items()))
        lines.append(f"RETRY {task.task_id} {retry_count} UNLESS-EXIT 85")
        lines.append("")
    for task in tasks:
        if task.parents:
    ...
    lines += [
        ...
        f"RETRY finalize_confirmatory {retry_count} UNLESS-EXIT 85", "",
    ]
    return "\n".join(lines)
...
def validate(tasks: list[Task], dag_text: str, output_dir: Path) -> dict:
    ...
    gpu_policy = submit_text["task_gpu.sub"]
    for required_policy in (
        ...
        if required_policy not in gpu_policy:
            raise RuntimeError(f"task_gpu.sub lacks GPU rematch policy: {required_policy}")
    if dag_text.count("\nJOB seed") != 62:
